...
def run_sweep(config, train_loader, val_loader, model=None):
    #config.WANDB_PROJECT= "NMA_Project_SER_sweep_test_v0"#"NMA_Project_SER_sweep_together_v1_e5"
    #print(config)
    EvaluationResult = config.EvaluationResult
    ...
    def train_sweep():
        run = wandb.init(config=config.CONFIG_DEFAULTS)
        #id_wandb=wandb.util.generate_id()
        #if not config.CUR_MODE == "benchmark":
        # wandb.init(
        # id=id_wandb,
    ...
    wandb.agent(sweep_id, function=train_sweep, count=config.N_SWEEP)
    wandb.finish()